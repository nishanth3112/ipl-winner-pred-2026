...
from config import RAW_DIR, MATCHES_CSV, TEAMS_JSON, TEAM_ALIASES, IPL_SOURCE_CSV

REQUIRED_COLUMNS = {
    "match_id", "innings", "batting_team", "bowling_team", "date", "season",
    "year", "match_won_by", "win_outcome", "toss_winner", "toss_decision",
    "venue", "city", "result_type", "stage", "batter", "runs_batter",
    "balls_faced", "player_out", "valid_ball", "bowler", "bowling_team",
    "runs_bowler", "bowler_wicket",
}
...
def build_all_matches(return_format: str = "legacy", dataset_path: str | None = None):
    ...
    """
    ipl_csv_path = resolve_ipl_csv_path(dataset_path)
    if not os.path.exists(ipl_csv_path):
        raise FileNotFoundError(
            f"IPL dataset not found at: {ipl_csv_path}. "
            "Pass --ipl-csv /path/to/file.csv or set IPL_CSV_PATH."
        )

    print(f"Loading ball-by-ball data from {ipl_csv_path}...")
    df = pd.read_csv(ipl_csv_path, low_memory=False)

    if df.columns.tolist() == ["version https://git-lfs.github.com/spec/v1"]:
        raise ValueError(
            "IPL.csv is a Git LFS pointer, not the actual dataset. "
            "Download the real file first. "
            "Expected a ball-by-ball CSV (~107 MB) with columns like 'match_id', "
            "'innings', 'batting_team', and 'bowling_team'."
        )

    missing_cols = sorted(REQUIRED_COLUMNS - set(df.columns))
    if missing_cols:
        raise ValueError(
            "IPL.csv does not match the expected ball-by-ball schema. "
            f"Missing columns: {missing_cols[:10]}"
